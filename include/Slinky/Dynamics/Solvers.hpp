...
{
    inline void SolveImpulses(const std::vector<std::unique_ptr<BodyContact>>& _contacts,
                              float _dt)
    {
    ...
    inline void SolvePositionsSmooth(const std::vector<std::unique_ptr<BodyContact>>& _contacts,
                               float _dt)
    {
        for (auto const &contact : _contacts) {
